...
def preprocess_features_for_tree(X_train, X_test_list, categorical_cols=None):
    """
...
def start_spark(app_name="xgb_training", master="local[*]"):   
    spark = (
...
def evaluate_model(model, X, y, threshold=0.5):
    ...
    if hasattr(model, 'predict_proba'):
        ...
        proba = model.predict(X, output_margin=False)
...
def main(args):
    ...
    spark = start_spark(app_name="loan_xgb_train")
    ...
    with mlflow.start_run(run_name=f"XGBoost_{args.train_date}"):
        # Log parameters
        mlflow.log_params({
            "model_type": "XGBoost",
        ...
        for name, X, y in [("train", X_train_xgb, y_train), ("val", X_val_xgb, y_val),
                           ("test", X_test_xgb, y_test), ("oot", X_oot_xgb, y_oot)]:
        ...
        mlflow.xgboost.log_model(best, artifact_path)
