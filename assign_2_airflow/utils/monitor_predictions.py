...
def read_preds(spark, path):
    ...
    'model_predictions'.  Tries several common column-name patterns and falls
    ...
    preferred = [
        "model_predictions", "model_prediction_proba", "churn_proba", "proba", "probability",
    ...
    if chosen is None:
        ...
        exclude = set(["Customer_ID", "snapshot_date", "id", "member_id", "userid"])
    ...
    # rename chosen column to 'model_predictions' if needed
    if chosen != "model_predictions":
        pdf = pdf.rename(columns={chosen: "model_predictions"})

    # sanity: ensure it's numeric
    pdf["model_predictions"] = pd.to_numeric(pdf["model_predictions"], errors="coerce")
    if pdf["model_predictions"].isnull().all():
...
def compute_metrics_with_labels(pdf_preds, labels_pdf, label_col="label"):
    ...
    merged = pdf_preds.merge(labels_pdf, on=["Customer_ID", "snapshot_date"], how="left")
    if label_col not in merged.columns:
        logger.warning(f"label column {label_col} not found in labels file")
        return None
    y_true = merged[label_col].astype(int)
    y_score = merged["model_predictions"].astype(float)
...
def main(args):
    ...
    # summaries
    cur_summary = summarize(cur["model_predictions"].values)
    base_summary = summarize(base["model_predictions"].values)

    # stat tests
    try:
        ks_stat, ks_p = ks_2samp(base["pmodel_predictions"].values, cur["model_predictions"].values)
    except Exception:
        ks_stat, ks_p = None, None

    psi_val = psi(base["model_predictions"].values, cur["model_predictions"].values, bins=10)

    # decile diffs table
    base_dec = np.quantile(base["model_predictions"].values, np.linspace(0, 1, 11))
    cur_dec = np.quantile(cur["model_predictions"].values, np.linspace(0, 1, 11))
    ...
    # optional label metrics - WITH ROBUST HANDLING
    if args.labels_path and args.labels_path.strip():  # Check if path is not empty
        ...
        try:
            ...
            if not os.path.exists(args.labels_path):
                logger.warning(f"Label store directory does not exist: {args.labels_path}")
                report["label_metrics"] = {"error": "label_store_directory_missing"}
            else:
                # Look for Parquet files
                parquet_files = glob.glob(os.path.join(args.labels_path, "*.parquet"))
                if not parquet_files:
                    logger.warning(f"No Parquet files found in label store: {args.labels_path}")
                    logger.info(f"Available files: {os.listdir(args.labels_path)}")
                    report["label_metrics"] = {"error": "no_parquet_files_in_label_store"}
                else:
                    # Try to load labels
                    files_list = [args.labels_path + os.path.basename(f) for f in parquet_files]
                    labels_sdf = spark.read.option("header", "true").parquet(*files_list)
                    labels_pdf = labels_sdf.toPandas()
                    
                    # Ensure proper date format if snapshot_date column exists
                    if 'snapshot_date' in labels_pdf.columns:
                        labels_pdf['snapshot_date'] = pd.to_datetime(labels_pdf['snapshot_date'])
                    
                    logger.info(f"✅ Loaded {len(labels_pdf)} labels")
                    
                    # Compute supervised metrics
                    supervised = compute_metrics_with_labels(cur, labels_pdf, label_col=args.label_col)
                    report["label_metrics"] = supervised if supervised else {"error": "supervised_metrics_computation_failed"}
                    
        except Exception as e:
            logger.error(f"❌ Error loading labels: {e}")
            report["label_metrics"] = {"error": f"label_loading_failed: {str(e)}"}
    else:
        logger.info("No labels path provided - skipping supervised metrics")
        report["label_metrics"] = {"info": "no_labels_provided"}
